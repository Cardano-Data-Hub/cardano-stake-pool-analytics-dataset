...
def generate_sql(file_path:str):
    """
...
def load_to_s3(copy_stmt, s3_partition, bucket='stakepool-analytics'):
    ...
    s3_resource.Object(bucket, s3_partition).put(Body=file_io.getvalue())

@app.command()
def get_kpi(file_name, interval='epoch'):
    FILE_PATH = generate_filepath(file_name, interval)
    S3_PARTITION = f"stakepool_kpis/{interval}/{file_name}.csv"
    COPY_STMT = generate_sql(FILE_PATH)
